...
set JIDDIR=c:/users/danie/desktop/mue_patran/cono/optimizacion/09
set MSC_APP=no
set MSC_AUTH=edu
set MSC_BASE=c:/program files/msc.software/napa_se/20211/nastran
set MSC_DBS=c:/scratch/09.T18296_22
set MSC_EXE='c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/analysis.exe'
set MSC_ISHELLPATH=c:/users/danie/desktop/mue_patran/cono/optimizacion/09
set MSC_ISHELLEXT=
set MSC_JID=C:\Users\danie\Desktop\MuE_Patran\Cono\Optimizacion\09\09.bdf
set MSC_JIDPATH=
set MSC_JIDTYPE=.bdf
set MSC_MEM=527MB
set MSC_OLD=yes
...
set MSC_VERSD=msc20211
set OUTDIR=.
set TMP=c:/users/danie/appdata/local/temp
...
set PWD=C:\Users\danie\Desktop\MuE_Patran\Cono\Optimizacion\09
set PWD=%PWD:/=\%
cd /d %PWD%
...
echo MSC Nastran V%VersionLocal% (Intel Windows 10 Pro 6.2 9200) Control File: %PRINTLOC%
...
set mksol700=no
if "%SOL700%" == "yes" (
   if not exist "./09.sol700.pth" (
      set mksol700=yes
      echo  "%MSC_BASE%\%MSC_VERSD%\dyna\win64\bin\dytrandmp.py" nastran > ./09.sol700.pth
      set  LSTC_VER=""
   )
)
set cmdline=C:\Program Files\MSC.Software\NaPa_SE\20211\Nastran\msc20211\win64i8\nastran.exe 09.bdf prog=edu auth=edu bat=no
...
if exist "09.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem 09.perf >> %lognt%
if exist "%sdirnt%\09.T18296_22.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem %sdirnt%\09.T18296_22.perf >> %lognt%
@rem
set /a f98 = 0
call :get_file_size  "%sdirnt%\09.T18296_22.f98"
if /i %FILE_SIZE% gtr 0 set /a f98 = 1
set /a f99 = 0
call :get_file_size  "%sdirnt%\09.T18296_22.f99"
...
if /i %i% neq 0 (
   ...
   if "%ferr%" == "" (
      if /i %f98% neq 0 (
         if /i %f99% neq 0 copy /f /y "%sdirnt%\09.T18296_22.f99"+"%sdirnt%\09.T18296_22.f98" "%outnt%.acms_out"
         if /i %f99% equ 0 copy /f /y "%sdirnt%\09.T18296_22.f98" "%outnt%.acms_out"
      ) else (
         copy /f /y "%sdirnt%\09.T18296_22.f99" "%outnt%.acms_out"
      )
   )
)
@rem
for %%f in ("%sdirnt%\09*_ts_*.dac") do call :delete_job_file "%%f"
for %%f in (dtout msg config) do call :concat_job_file %%f
for %%f in ("%sdirnt%\09.T18296_22.*") do call :delete_job_file "%%f"
...
if exist "09.perf" del /f "09.perf"
if "%mksol700%" == "yes" (
   if exist ./09.sol700.pth del /f ./09.sol700.pth
)
...
setlocal
set ext=%~x1
type "09_SN_*.%1" > 09.%1 2>nul
del /f "09_SN_*.%1" 2>nul:
>nul findstr "^" "09.%1" || del "09.%1"
endlocal
goto :eof
@rem
@rem
