@rem  BAT file generated from the c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/nastran.lcl
...
set outnt=./03
set outnt=%outnt:/=\%
set archdir=c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8
set archdir=%archdir:/=\%
set archdir=%archdir:win64i4=win64i8%
set MKL_INTERFACE_LAYER=ILP64 
set datecmd=c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/mscdate
set datecmd=%datecmd:/=\%
set exedir=
set jobnt=./03.T11860_13
set jobnt=%jobnt:/=\%
set lognt=./03.log
set lognt=%lognt:/=\%
set modedir=
set nastcmd=c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/nastran.exe
set nastcmd=%nastcmd:/=\%
set nastcmd=%nastcmd:win64i4=win64i8%
set sdirnt=c:/scratch
set sdirnt=%sdirnt:/=\%
set solver=c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/analysis.exe
set solver=%solver:/=\%
set tcmdopt=
set tcmd=c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/msctime
set tcmd=%tcmd:/=\%
set user=Usuario
set MSC_BASE=c:/program files/msc.software/napa_se/20211/nastran
set MSC_VERSD=msc20211
...
set JIDDIR=c:/users/usuario/desktop/mue_patran/cono/optimizacion/04
set MSC_APP=no
set MSC_AUTH=edu
set MSC_BASE=c:/program files/msc.software/napa_se/20211/nastran
set MSC_DBS=c:/scratch/03.T11860_13
set MSC_EXE='c:/program files/msc.software/napa_se/20211/nastran/msc20211/win64i8/analysis.exe'
set MSC_ISHELLPATH=c:/users/usuario/desktop/mue_patran/cono/optimizacion/04
set MSC_ISHELLEXT=
set MSC_JID=C:\Users\Usuario\Desktop\MuE_Patran\Cono\Optimizacion\04\03.bdf
set MSC_JIDPATH=
set MSC_JIDTYPE=.bdf
set MSC_MEM=527MB
set MSC_OLD=yes
...
set MSC_VERSD=msc20211
set OUTDIR=.
set TMP=c:/users/usuario/appdata/local/temp
set MSC_SQAS_MESSAGE_FILE='c:/program files/msc.software/napa_se/20211/nastran\msc20211\win64i8\analysis.msg'
...
set PWD=C:\Users\Usuario\Desktop\MuE_Patran\Cono\Optimizacion\04
set PWD=%PWD:/=\%
cd /d %PWD%
...
set VersionLocal=2021.1
set MAIN=%VersionLocal:~0,4%
set SUB=%VersionLocal:~4,2%
set NEW=%VersionLocal:.=.%
if NOT %SUB%==.0 set VersionLocal=%NEW%
if     %SUB%==.0 set VersionLocal=%MAIN%
echo MSC Nastran V%VersionLocal% (Intel Windows 10 Enterprise 6.2 9200) Control File: %PRINTLOC%
...
set mksol700=no
if "%SOL700%" == "yes" (
   if not exist "./03.sol700.pth" (
      set mksol700=yes
      echo  "%MSC_BASE%\%MSC_VERSD%\dyna\win64\bin\dytrandmp.py" nastran > ./03.sol700.pth
      set  LSTC_VER=""
   )
)
set cmdline=C:\Program Files\MSC.Software\NaPa_SE\20211\Nastran\msc20211\win64i8\nastran.exe 03.bdf prog=edu auth=edu bat=no
...
if exist "03.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem 03.perf >> %lognt%
if exist "%sdirnt%\03.T11860_13.perf" "c:/program files/msc.software/napa_se/20211/nastran/bin/msc20211" mscsystem %sdirnt%\03.T11860_13.perf >> %lognt%
@rem
set /a f98 = 0
call :get_file_size  "%sdirnt%\03.T11860_13.f98"
if /i %FILE_SIZE% gtr 0 set /a f98 = 1
set /a f99 = 0
call :get_file_size  "%sdirnt%\03.T11860_13.f99"
...
if /i %i% neq 0 (
   ...
   if "%ferr%" == "" (
      if /i %f98% neq 0 (
         if /i %f99% neq 0 copy /f /y "%sdirnt%\03.T11860_13.f99"+"%sdirnt%\03.T11860_13.f98" "%outnt%.acms_out"
         if /i %f99% equ 0 copy /f /y "%sdirnt%\03.T11860_13.f98" "%outnt%.acms_out"
      ) else (
         copy /f /y "%sdirnt%\03.T11860_13.f99" "%outnt%.acms_out"
      )
   )
)
@rem
for %%f in ("%sdirnt%\03*_ts_*.dac") do call :delete_job_file "%%f"
for %%f in (dtout msg config) do call :concat_job_file %%f
for %%f in ("%sdirnt%\03.T11860_13.*") do call :delete_job_file "%%f"
...
if exist "03.perf" del /f "03.perf"
if "%mksol700%" == "yes" (
   if exist ./03.sol700.pth del /f ./03.sol700.pth
)
...
setlocal
set ext=%~x1
type "03_SN_*.%1" > 03.%1 2>nul
del /f "03_SN_*.%1" 2>nul:
>nul findstr "^" "03.%1" || del "03.%1"
endlocal
goto :eof
@rem
@rem
